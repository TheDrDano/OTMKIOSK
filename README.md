...
```powershell
.\scripts\build-installer.ps1 -Version 9.3.1
```
...
Use one lowercase release tag per version, like `v9.3.1`. The workflow normalizes manual inputs and `V...` tags back to lowercase `v...`, but old duplicate GitHub releases should be deleted from the GitHub **Releases** page once so the list stays clean.
...
```powershell
.\scripts\build-installer.ps1 -Version 9.3.1 -Sign -CertificateThumbprint "YOUR_CERT_THUMBPRINT"
```
...
```powershell
.\scripts\build-installer.ps1 -Version 9.3.1 -Sign -PfxPath "C:\secure\otm-signing.pfx" -PfxPassword "PFX_PASSWORD"
```
...
.\scripts\build-installer.ps1 -Version 9.3.1 -Sign -PfxPath ".\artifacts\signing\simplekioskos-test.pfx" -PfxPassword "test-password"
```
